...
public class SharedWaypointsClient implements ClientModInitializer {
							...
							if (Thread.currentThread().getName().equals("Render thread")) {
								clientPlayNetworkHandler.sendPacket(
									ClientPlayNetworking.createC2SPacket(
										sWIdentifier,
										SharedWaypointsClient.packetToByteBuf(new SWRegister(SharedWaypointsClient.protocolVersion))
									)
								);
							}
						} catch (IOException e) {
	...
	public static void sendPacket(SWPacket packet) {
		try {
			...
			handler.sendPacket(ClientPlayNetworking.createC2SPacket(sWIdentifier, SharedWaypointsClient.packetToByteBuf(packet)));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
